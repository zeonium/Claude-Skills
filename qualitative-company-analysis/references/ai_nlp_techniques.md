...
## A5: FOG Lexicon Scorer (Rittenhouse Net Candor Score)

### 원리
Rittenhouse (2013) FOG 분류 + 가점 룰을 정규식 사전으로 자동 채점하여 Net Candor Score / Candor Index / Communication Efficiency를 산출.

### 핵심 산식

```
Net Candor Score = Σ가점 - Σ감점
가점: +5(정성 목표) | +10(정량 목표) | +5(목표 맥락) | +3(Cash) | +3(FCF 맥락)
감점: -3(상투어) | -3(위즐워드) | -5(오웰 문장) | -10(전략 부조화)

FOG 비율     = Σ감점 / (Σ가점 + Σ감점)
Candor Index = (1 - FOG 비율) × 100   [0~100]
```

### scripts/fog_lexicon_scorer.py 연결

```bash
# 영어 서한
python3 scripts/fog_lexicon_scorer.py \
  --input letter_FY2024.txt \
  --lang en --company NVIDIA --out nvda_fog

# 한국어 서한
python3 scripts/fog_lexicon_scorer.py \
  --input letter_FY2024.txt \
  --lang ko --company 삼성전자 --out samsung_fog

# 엔론 6종 세트 패턴 탐지
python3 scripts/fog_lexicon_scorer.py \
  --input letter.txt --lang en --detect-enron-pattern --out check
```

**D1 연동**: Candor Index → `templates/management_credibility_scorecard.md` §CANDOR-7 통합 섹션.
**Candor Crash 트리거**: 오웰 문장 1건 이상 또는 엔론 6종 세트 감지 시 D1 -10 페널티.

상세: `references/candor7_framework.md`, `references/fog_lexicon.md`

---

## A6: CEO Letter Forensics (자기집필도 / FCF 맥락 / 실수 대처)

### 원리
Rittenhouse 5단계 포렌식 중 STEP 3 (FCF 맥락), STEP 5 (실수 대처), 자기집필도를 정규식·키워드 빈도로 진단.

### 4가지 체크

| 옵션 | 측정 | 가중치 영향 |
|------|------|-----------|
| `--check authorship` | 1인칭 빈도 + 일화 + 실수 인정 | 자기집필 점수 0-100 |
| `--check fcf` | FCF 정의·5년 대조·사용처 | ① Capital Stewardship +2 |
| `--check mistake` | 인정 vs 외부 탓 비율 | ⑤ Leadership +2/-1 |
| `--check orwell` | 오웰 모순 패턴 | -5 × N |
| `--check all` | 4가지 모두 | 전부 |

### scripts/ceo_letter_forensics.py 연결

```bash
# 전체 진단 (기본값)
python3 scripts/ceo_letter_forensics.py \
  --input letter.txt --lang en --company NVIDIA --out nvda_forensics

# FCF만 (J&J Larsen 패턴 검증)
python3 scripts/ceo_letter_forensics.py \
  --input letter.txt --lang ko --check fcf
```

**Investment Memo 연동**: 자기집필 점수 < 30 시 Step 5 Pre-mortem에서 "IR 대행 가설" 검증 필수.
...
Step 3A → A5 FOG Scorer + A6 Letter Forensics (CEO 주주 서한)
Step 3B → A2 Tone Analysis + A3 Moving Targets (컨퍼런스콜)
